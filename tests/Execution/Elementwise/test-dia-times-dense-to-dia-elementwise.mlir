...
module {
  ...
  func.func @test_batch() {
    ...
    %add_res = dia.elementwise kind = <add> ins(%diaA, %denseB : tensor<2x4x5xf32>, tensor<2x5x5xf32>) 
                                            outs(%0 : tensor<2x5x5xf32>) -> tensor<2x5x5xf32>
                                      
    %memref = memref.alloc() : memref<2x5x5xf32>
    bufferization.materialize_in_destination %add_res in %memref {writable} : (tensor<2x5x5xf32>, memref<2x5x5xf32>) -> ()
    %cast = memref.cast %memref : memref<2x5x5xf32> to memref<*xf32>
    call @printMemrefF32(%cast) : (memref<*xf32>) -> ()
    memref.dealloc %memref : memref<2x5x5xf32>
