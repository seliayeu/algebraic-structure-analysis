// RUN: %build/tools/alg-opt %s --banded-analysis --banded-rewrite \
// RUN:  --reconcile-unrealized-casts \
// RUN:  --one-shot-bufferize="allow-return-allocs-from-loops=true bufferize-function-boundaries=true" \
// RUN:  --convert-linalg-to-loops --convert-scf-to-cf --convert-cf-to-llvm \
// RUN:  --convert-arith-to-llvm --finalize-memref-to-llvm --convert-func-to-llvm \
// RUN:  --reconcile-unrealized-casts | \
// RUN:  mlir-runner --entry-point-result=void \
// RUN:  --shared-libs=%llvm_root/build/lib/libmlir_runner_utils.%lib_format > %t
// RUN: FileCheck %s < %t

module {
  func.func private @printMemrefF32(memref<*xf32>)

  // CHECK: Unranked Memref {{.*}} rank = 2 offset = 0 sizes = [3, 3]
  // CHECK: {{\[}}[4{{.*}}, 4{{.*}}, 4{{.*}}],
  // CHECK-NEXT: [6{{.*}}, 6{{.*}}, 6{{.*}}],
  // CHECK-NEXT: [4{{.*}}, 4{{.*}}, 4{{.*}}]]
  func.func @regular() {
    %I0 = arith.constant {metadata = {lowerBw = 1 : i64, upperBw = 1 : i64, propertyDims = [0, 1]}}
        dense<1.0> : tensor<3x3xf32>
    %I1 = arith.constant {metadata = {lowerBw = 2 : i64, upperBw = 2 : i64, propertyDims = [0, 1]}}
        dense<2.0> : tensor<3x3xf32>
    %e1 = arith.constant dense<0.0> : tensor<3x3xf32>

    %R0 = linalg.matmul
            ins(%I0, %I1 : tensor<3x3xf32>, tensor<3x3xf32>)
              outs(%e1 : tensor<3x3xf32>) -> tensor<3x3xf32>

    %memref = memref.alloc() : memref<3x3xf32>
    bufferization.materialize_in_destination %R0 in %memref {writable}
        : (tensor<3x3xf32>, memref<3x3xf32>) -> ()
    %cast = memref.cast %memref : memref<3x3xf32> to memref<*xf32>
    call @printMemrefF32(%cast) : (memref<*xf32>) -> ()
    memref.dealloc %memref : memref<3x3xf32>
    return
  }
  // CHECK: Unranked Memref {{.*}} rank = 2 offset = 0 sizes = [3, 3]
  // CHECK: {{\[}}[4{{.*}}, 4{{.*}}, 0{{.*}}],
  // CHECK-NEXT: [6{{.*}}, 6{{.*}}, 6{{.*}}],
  // CHECK-NEXT: [0{{.*}}, 4{{.*}}, 4{{.*}}]]
  func.func @l1_u1_output() {
    %I0 = arith.constant {metadata = {lowerBw = 1 : i64, upperBw = 1 : i64, propertyDims = [0, 1]}}
        dense<1.0> : tensor<3x3xf32>
    %I1 = arith.constant {metadata = {lowerBw = 2 : i64, upperBw = 2 : i64, propertyDims = [0, 1]}}
        dense<2.0> : tensor<3x3xf32>
    %e1 = arith.constant dense<0.0> : tensor<3x3xf32>

    %R0 = linalg.matmul {metadata = {lowerBw = 1: i64, propertyDims = [0, 1], upperBw = 1: i64}}
            ins(%I0, %I1 : tensor<3x3xf32>, tensor<3x3xf32>)
              outs(%e1 : tensor<3x3xf32>) -> tensor<3x3xf32>

    %memref = memref.alloc() : memref<3x3xf32>
    bufferization.materialize_in_destination %R0 in %memref {writable}
        : (tensor<3x3xf32>, memref<3x3xf32>) -> ()
    %cast = memref.cast %memref : memref<3x3xf32> to memref<*xf32>
    call @printMemrefF32(%cast) : (memref<*xf32>) -> ()
    memref.dealloc %memref : memref<3x3xf32>
    return
  }

  // CHECK: Unranked Memref {{.*}} rank = 2 offset = 0 sizes = [3, 3]
  // CHECK: {{\[}}[4{{.*}}, 0{{.*}}, 0{{.*}}],
  // CHECK-NEXT: [0{{.*}}, 6{{.*}}, 0{{.*}}],
  // CHECK-NEXT: [0{{.*}}, 0{{.*}}, 4{{.*}}]]
  func.func @diagonal_output() {
    %I0 = arith.constant {metadata = {lowerBw = 1 : i64, upperBw = 1 : i64, propertyDims = [0, 1]}}
        dense<1.0> : tensor<3x3xf32>
    %I1 = arith.constant {metadata = {lowerBw = 2 : i64, upperBw = 2 : i64, propertyDims = [0, 1]}}
        dense<2.0> : tensor<3x3xf32>
    %e1 = arith.constant dense<0.0> : tensor<3x3xf32>

    %R0 = linalg.matmul {metadata = {lowerBw = 0: i64, propertyDims = [0, 1], upperBw = 0: i64}}
            ins(%I0, %I1 : tensor<3x3xf32>, tensor<3x3xf32>)
              outs(%e1 : tensor<3x3xf32>) -> tensor<3x3xf32>

    %memref = memref.alloc() : memref<3x3xf32>
    bufferization.materialize_in_destination %R0 in %memref {writable}
        : (tensor<3x3xf32>, memref<3x3xf32>) -> ()
    %cast = memref.cast %memref : memref<3x3xf32> to memref<*xf32>
    call @printMemrefF32(%cast) : (memref<*xf32>) -> ()
    memref.dealloc %memref : memref<3x3xf32>
    return
  }

  func.func @main() {
    call @regular() : () -> ()
    call @l1_u1_output() : () -> ()
    call @diagonal_output() : () -> ()
    return 
  }
}

