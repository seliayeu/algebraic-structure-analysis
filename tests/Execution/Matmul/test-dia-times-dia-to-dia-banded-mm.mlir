...
module {
  ...
  func.func @l1u1_l1u1_forced() {
    %0 = tensor.empty() : tensor<3x3xf32>
    %dia1 = arith.constant {metadata = {dia = true, upperBw = 1 : i64, lowerBw = 1 : i64, propertyDims = [0, 1]}}
        dense<[[0.0, 1.0, 1.0],
               [1.0, 2.0, 1.0],
               [1.0, 1.0, 0.0]]> : tensor<3x3xf32>

    %dia2 = arith.constant {metadata = {dia = true, upperBw = 1 : i64, lowerBw = 1 : i64, propertyDims = [0, 1]}}
        dense<[[0.0, 1.0, 1.0],
               [1.0, 2.0, 1.0],
               [1.0, 1.0, 0.0]]> : tensor<3x3xf32>
    ...
    // CHECK-NEXT: [2, 6, 2]
    // CHECK-NEXT: [3, 3, 0]]
    %1 = dia.matmul ins(%dia1, %dia2 : tensor<3x3xf32>, tensor<3x3xf32>)
                    outs(%0 : tensor<3x3xf32>) -> tensor<3x3xf32> {metadata = {dia = true, upperBw = 1 : i64, lowerBw = 1 : i64, propertyDims = [0, 1]}}
  ...
  func.func @l1u1_l1u1() {
    ...
    // CHECK-NEXT: [1, 0, 0]]
    %1 = dia.matmul ins(%dia1, %dia2 : tensor<3x3xf32>, tensor<3x3xf32>)
                    outs(%0 : tensor<5x3xf32>) -> tensor<5x3xf32>

    %memref = memref.alloc() : memref<5x3xf32>
    bufferization.materialize_in_destination %1 in %memref {writable} : (tensor<5x3xf32>, memref<5x3xf32>) -> ()
    %cast = memref.cast %memref : memref<5x3xf32> to memref<*xf32>

    call @printMemrefF32(%cast) : (memref<*xf32>) -> ()
    memref.dealloc %memref : memref<5x3xf32>
    return
  }

  func.func @l1u0_l0u1() {
    %0 = tensor.empty() : tensor<3x3xf32>
    %dia1 = arith.constant {metadata = {dia = true, upperBw = 1 : i64, lowerBw = 0 : i64, propertyDims = [0, 1]}}
        dense<[[1.0, 1.0, 1.0],
               [1.0, 2.0, 0.0]]> : tensor<2x3xf32>
    %dia2 = arith.constant {metadata = {dia = true, upperBw = 0 : i64, lowerBw = 1 : i64, propertyDims = [0, 1]}}
        dense<[[0.0, 1.0, 2.0],
               [1.0, 2.0, 1.0]]> : tensor<2x3xf32>
    // CHECK:      Unranked Memref
    // CHECK-SAME: sizes = [3, 3]
    // CHECK-SAME: data =
    // CHECK-NEXT: {{\[\[}}0, 1, 2],
  ...
  func.func @l1u0_l1u0() {
    ...
    %dia1 = arith.constant {metadata = {dia = true, upperBw = 1 : i64, lowerBw = 0 : i64, propertyDims = [0, 1]}}
        dense<[[1.0, 1.0, 1.0],
               [1.0, 2.0, 0.0]]> : tensor<2x3xf32>
    %dia2 = arith.constant {metadata = {dia = true, upperBw = 1 : i64, lowerBw = 0 : i64, propertyDims = [0, 1]}}
        dense<[[1.0, 1.0, 2.0],
               [1.0, 2.0, 0.0]]> : tensor<2x3xf32>
    // CHECK:      Unranked Memref
    // CHECK-SAME: sizes = [3, 3]
