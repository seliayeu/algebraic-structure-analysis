...
// CHECK: %cst = arith.constant {metadata = {lowerBw = 0 : i64, propertyDims = [0, 1], upperBw = 0 : i64}} dense<{{\[\[}}1.{{.*}}, 2.{{.*}}, 3.{{.*}}]]> : tensor<1x3xf32>
...
// CHECK: %cst = arith.constant {metadata = {lowerBw = 0 : i64, propertyDims = [0, 1], upperBw = 1 : i64}} dense<{{\[\[}}1.{{.*}}, 3.{{.*}}, 5.{{.*}}], [0.{{.*}}, 2.{{.*}}, 4.{{.*}}]]> : tensor<2x3xf32>
// CHECK-NOT: dia.from_dense
...
// CHECK: %cst = arith.constant {metadata = {lowerBw = 1 : i64, propertyDims = [0, 1], upperBw = 0 : i64}} dense<{{\[\[}}2.{{.*}}, 4.{{.*}}, 0.{{.*}}], [1.{{.*}}, 3.{{.*}}, 5.{{.*}}]]> : tensor<2x3xf32>
// CHECK-NOT: dia.from_dense
...
// CHECK: %cst = arith.constant {metadata = {lowerBw = 1 : i64, propertyDims = [0, 1], upperBw = 1 : i64}} dense<{{\[\[}}3.{{.*}}, 6.{{.*}}, 0.{{.*}}], [1.{{.*}}, 4.{{.*}}, 7.{{.*}}], [0.{{.*}}, 2.{{.*}}, 5.{{.*}}]]> : tensor<3x3xf32>
// CHECK-NOT: dia.from_dense
