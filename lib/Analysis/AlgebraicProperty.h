...
static AlgebraicProperty meet(const AlgebraicProperty lhs, const AlgebraicProperty rhs) {
    if (lhs == AlgebraicProperty::Identity) return rhs;
    if (rhs == AlgebraicProperty::Identity) return lhs;
    if (lhs == AlgebraicProperty::Diagonal) return rhs;
    if (rhs == AlgebraicProperty::Diagonal) return lhs;
...
static AlgebraicProperty join(const AlgebraicProperty& lhs, const AlgebraicProperty& rhs) {
    if (lhs == AlgebraicProperty::General) return rhs;
    if (rhs == AlgebraicProperty::General) return lhs;
    if (lhs == AlgebraicProperty::Symmetric || lhs == AlgebraicProperty::UpperTriangular ||
        lhs == AlgebraicProperty::LowerTriangular)
        return AlgebraicProperty::Diagonal;
    if (rhs == AlgebraicProperty::Symmetric || rhs == AlgebraicProperty::UpperTriangular ||
        rhs == AlgebraicProperty::LowerTriangular)
        return AlgebraicProperty::Diagonal;
    if (lhs == AlgebraicProperty::Diagonal) return rhs;
    if (rhs == AlgebraicProperty::Diagonal) return lhs;
    if (lhs == AlgebraicProperty::Identity) return rhs;
...
static AlgebraicProperty binaryMatmul(AlgebraicProperty lhs, AlgebraicProperty rhs) {
    if (lhs == AlgebraicProperty::Identity && lhs == rhs) return AlgebraicProperty::Identity;
    if (lhs == AlgebraicProperty::Diagonal && lhs == rhs) return AlgebraicProperty::Diagonal;
...
static AlgebraicProperty binaryElementwiseGeneral(AlgebraicProperty lhs, AlgebraicProperty rhs) {
    if (lhs == AlgebraicProperty::Identity && lhs == rhs) return AlgebraicProperty::Diagonal;
    if (lhs == AlgebraicProperty::Diagonal && lhs == rhs) return AlgebraicProperty::Diagonal;
    if (lhs == AlgebraicProperty::Symmetric && lhs == rhs) return AlgebraicProperty::Symmetric;
    if (lhs == AlgebraicProperty::LowerTriangular && lhs == rhs)
        return AlgebraicProperty::LowerTriangular;
    if (lhs == AlgebraicProperty::UpperTriangular && lhs == rhs)
        return AlgebraicProperty::UpperTriangular;
...
static AlgebraicProperty unaryElementwise(AlgebraicProperty property) {
    if (property == AlgebraicProperty::Identity) return AlgebraicProperty::Diagonal;
    return property;
}

static AlgebraicProperty transpose(AlgebraicProperty property) {
    if (property == AlgebraicProperty::UpperTriangular) return AlgebraicProperty::LowerTriangular;
    if (property == AlgebraicProperty::LowerTriangular) return AlgebraicProperty::UpperTriangular;
    return property;
}
