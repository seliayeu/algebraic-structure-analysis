...
SubMatrixProperty AlgebraicStructureAnalysis::readPropertyFromDictAttr(DictionaryAttr dictAttr) {
    ...
    if (!metadataAttr) return { AlgebraicProperty::General };
    auto innerDictAttr{ dyn_cast<DictionaryAttr>(metadataAttr) };
    if (!innerDictAttr) return { AlgebraicProperty::General };
    auto analysisPropertyAttr{ innerDictAttr.get("analysisState") };
    if (!analysisPropertyAttr) return { AlgebraicProperty::General };
    ...
    if (!propertyDimsAttr) return { AlgebraicProperty::General };
...
LogicalResult AlgebraicStructureAnalysis::visitMatmul(linalg::MatmulOp* op) {
    ...
    if (indexingMapArr.size() != 3) return success();
    ...
    if (indexingMapArr[0] != expectedMapLhs || indexingMapArr[1] != expectedMapRhs ||
...
LogicalResult AlgebraicStructureAnalysis::visitAdd(linalg::AddOp* op) {
    ...
    if (propertyMap[lhs].dimensions != propertyMap[rhs].dimensions) return success();
...
LogicalResult AlgebraicStructureAnalysis::visitTranspose(linalg::TransposeOp* op) {
    ...
    auto& dims{ propertyMap[operands[0]].dimensions };
    auto dim0{ std::find(permutation.begin(), permutation.end(), dims[0]) };
    auto dim1{ std::find(permutation.begin(), permutation.end(), dims[1]) };

    if (*dim0 < *dim1) {
        auto newProperty{ propertyMap[operands[0]].property };
        propertyMap[result] = { join(propertyMap[result].property, newProperty), { *dim0, *dim1 } };
        return success();
    }

    auto newProperty{ transpose(propertyMap[operands[0]].property) };
    propertyMap[result] = { join(propertyMap[result].property, newProperty), { *dim1, *dim0 } };
...
LogicalResult AlgebraicStructureAnalysis::visitGeneric(linalg::GenericOp* op) {
    ...
    if (kExpr != rhsMap.getResult(rhsProp.dimensions[0])) return success();
    ...
    if (body->getOperations().size() != 3 || body->getNumArguments() != 3) return success();

    auto accumulator{ body->getArgument(2) };

    auto yieldOp{ cast<linalg::YieldOp>(body->getTerminator()) };
    auto yieldOperands{ yieldOp->getOperands() };
    if (yieldOperands.size() != 1) return success();
    auto addOp{ yieldOperands[0].getDefiningOp() };
    if (!addOp || !isa<arith::AddIOp, arith::AddFOp>(addOp)) return success();
    Operation* mulOp;
    ...
    if (!mulOp || !isa<arith::MulIOp, arith::MulFOp>(mulOp)) return success();

    if (!((mulOp->getOperand(0) == body->getArgument(1) &&
    ...
    propertyMap[result] =
        SubMatrixProperty{ newProperty, { mResultIdx.value(), nResultIdx.value() } };
